...
#[cfg(test)]
use crate::test_common::{get_vec_tree_from_slice, XOR128};
...
pub struct Proof<T: ArrayLength, BaseTreeArity: Unsigned = U2> {
    ...
    lemma: Vec<GenericArray<u8, T>>,
    path: Vec<usize>, // branch index

    _u: PhantomData<BaseTreeArity>, // number of branches per node
}

impl<T: ArrayLength, BaseTreeArity: Unsigned> Proof<T, BaseTreeArity> {
    ...
    fn validate_sub_tree_proof<A: Algorithm<OutputSize = T>>(&self, arity: usize) -> Result<bool> {
        ...
        let h = {
            let mut nodes: Vec<_> = Vec::with_capacity(node_count);
            let mut cur_index = 0;
    ...
    pub fn validate<A: Algorithm<OutputSize = T>>(&self) -> Result<bool> {
        ...
        for i in (1..size - 1).step_by(branches - 1) {
            a.reset();
            h = {
                let mut nodes: Vec<_> = Vec::with_capacity(branches);
                let mut cur_index = 0;
                for j in 0..branches {
    ...
    pub fn validate_with_data<A: Algorithm<OutputSize = T>>(
        &self,
        leaf_data: &impl AsRef<[u8]>,
    ) -> Result<bool> {
        let mut a = A::new();
        a.update(leaf_data);
        let item = a.finalize_reset();
    ...
    pub fn lemma(&self) -> &Vec<GenericArray<u8, T>> {
        &self.lemma
    }

    /// Returns the lemma of this proof as mutable.
    pub fn lemma_mut(&mut self) -> &mut Vec<GenericArray<u8, T>> {
        &mut self.lemma
    }
...
fn modify_proof<BaseTreeArity: Unsigned, SubTreeArity: Unsigned, TopTreeArity: Unsigned>(
    proof: &mut Proof<typenum::U16, BaseTreeArity>,
) {
    use digest::Digest;
    use rand::prelude::*;
    ...
    if TopTreeArity::to_usize() > 0 || SubTreeArity::to_usize() > 0 {
        ...
        a.update(&j.to_le_bytes());

        // Break random sub-tree proof element
        a.update(proof.sub_tree_proof.as_ref().unwrap().lemma()[i]);
        proof.sub_tree_proof.as_mut().unwrap().lemma_mut()[i] = a.finalize();
    } else {
        let i = random::<usize>() % proof.lemma.len();
        let k = random::<usize>();

        let mut a = XOR128::new();
        a.update(&k.to_le_bytes());

        // Break random element
        a.update(&proof.lemma[i]);
        proof.lemma[i] = a.finalize();
    }
}
...
fn test_compound_single_octree_broken_proofs() {
    ...
    let tree: MerkleTree<XOR128, VecStore<_>, U8, U1> =
...
fn test_compound_octree_broken_proofs() {
    ...
    let tree: MerkleTree<XOR128, VecStore<_>, U8, U4> =
...
fn test_ccompound_quad_broken_proofs() {
    ...
    let mt2 = get_vec_tree_from_slice::<U4>(leafs);
    let mt3 = get_vec_tree_from_slice::<U4>(leafs);
    let cmt1: MerkleTree<XOR128, VecStore<_>, U4, U3> =
    ...
    let cmt2: MerkleTree<XOR128, VecStore<_>, U4, U3> =
    ...
    let cmt3: MerkleTree<XOR128, VecStore<_>, U4, U3> =
        MerkleTree::from_trees(vec![mt7, mt8, mt9]).expect("failed to build compound merkle tree");

    let tree: MerkleTree<XOR128, VecStore<_>, U4, U3, U3> =
...
fn test_ccompound_single_quad_broken_proofs() {
    ...
    let tree: MerkleTree<XOR128, VecStore<_>, U4, U3, U1> =
...
fn test_ccompound_octree_broken_proofs() {
    ...
    let mt3 = get_vec_tree_from_slice::<U8>(leafs);
    let mt4 = get_vec_tree_from_slice::<U8>(leafs);
    let cmt1: MerkleTree<XOR128, VecStore<_>, U8, U4> =
        MerkleTree::from_trees(vec![mt1, mt2, mt3, mt4]).expect("Failed to build compound tree");
    ...
    let cmt2: MerkleTree<XOR128, VecStore<_>, U8, U4> =
    ...
    let cmt3: MerkleTree<XOR128, VecStore<_>, U8, U4> =
    ...
    let cmt4: MerkleTree<XOR128, VecStore<_>, U8, U4> =
    ...
    let cmt5: MerkleTree<XOR128, VecStore<_>, U8, U4> =
        MerkleTree::from_trees(vec![mt17, mt18, mt19, mt20])
            .expect("Failed to build compound tree");

    let tree: MerkleTree<XOR128, VecStore<_>, U8, U4, U5> =
...
fn test_ccompound_single_octree_broken_proofs() {
    ...
    let tree: MerkleTree<XOR128, VecStore<_>, U8, U4, U1> =
        MerkleTree::from_sub_trees(vec![cmt1]).expect("Failed to build ccompound tree");
